...
export default function CandidatosAlcaldesCard() {
  const [totalCandidatos, setTotalCandidatos] = useState(0);

  useEffect(() => {
    const candidatos = dataService.getCandidatos('alcalde');
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl md:rounded-3xl border border-slate-200 dark:border-gray-700 p-4 md:p-5 hover:shadow-md transition-all">
      <div className="flex items-center justify-between mb-3 md:mb-4">
        <p className="text-xs md:text-sm font-bold text-slate-600 dark:text-gray-300">Candidatos Alcaldes</p>
        <UserCheck className="w-5 h-5 md:w-6 md:h-6 text-slate-900 dark:text-white" />
      </div>

...
export default function ReclamacionesCard() {
  ...
  useEffect(() => {
    ...
    if (reclamacionesGuardadas) {
      const reclamaciones = JSON.parse(reclamacionesGuardadas);
      setTotalReclamaciones(reclamaciones.length);
    }

    // Escuchar cambios en las reclamaciones
    ...
    window.addEventListener('reclamacionesUpdated', handleStorageChange);

    return () => {
      window.removeEventListener('storage', handleStorageChange);
      window.removeEventListener('reclamacionesUpdated', handleStorageChange);
    };
  }, []);
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl md:rounded-3xl border border-slate-200 dark:border-gray-700 p-4 md:p-5 hover:shadow-md transition-all">
      <div className="flex items-start justify-between mb-3 md:mb-4">
        <p className="text-xs md:text-sm font-bold text-slate-600 dark:text-gray-300">Reclamaciones</p>
        <BookOpenText className="w-5 h-5 md:w-6 md:h-6 text-slate-900 dark:text-white" />
      </div>
      
      <p className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-white mb-1.5 md:mb-2">{totalReclamaciones}</p>
